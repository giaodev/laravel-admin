...
<div class="row">
    ...
    foreach($gallery as $image){
      ...
      if($image != NULL){
        @endphp
        <div data-target="#carousel-example-generic" data-slide-to="{{ $stt - 1 }}" class="{{ ($stt == 0) ? "active" : "" }} col-sm-3 col-xs-3">
          <a class="thumbnail">
            <img src="{{ $image }}" alt="...">
        </a>
    </div>
...
<div class="col-sm-3 dm-list-right">
    <li class="ds-item price_ct">
        ...
        <p></span><br><span style="color:#c70000; font-weight:bold; font-size:18px;">GIÁ: {{ number_format($data->product_price) }} VNĐ</span></p>
        @else
        <p><span style="text-decoration:line-through; font-weight:bold">Giá: {{ number_format($data->product_price) }} VNĐ</span><br><span style="color:#c70000; font-weight:bold; font-size:18px;">GIÁ KM: {{ number_format($data->product_promotion) }} VNĐ</span></p>
        @endif
...
<div class="order-now">
    <div class="">
      <button class="bg_booksp" title="Quick Orders" data-toggle="modal" data-target=".bs-example-modal-lg"><span style="color:#FFF; text-transform:uppercase">Đặt hàng nhanh</span><br><span>Mua hàng nhanh chóng, không cần đăng ký. Giao hàng Miễn Phí</span>
      </button>
  </div>
</div>

<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" id="quick_orders">
        ...
        <div class="row">
            ...
            <div class="col-sm-7">
                <form role="form">
                    ...
                    <button type="submit" class="btn btn-default orders_submit">Đặt hàng ngay</button>
                </form>
